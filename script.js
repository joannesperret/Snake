window.onload = function () {
  ...
  function Apple(position) {
    ...
    this.setNewPosition = function () {
      // Correction de la position de la pomme qui était définie en dehors du cadre
      // Ajout d'un contrôle sur posotion minimale
      var newX = Math.round(Math.random() * widthinBlock - 1);
      if (newX <= 0) { newX = Math.round(Math.random() * heightinBlock - 1) }
      var newY = Math.round(Math.random() * heightinBlock - 1);
      if (newY <= 0) { newY = Math.round(Math.random() * heightinBlock - 1) }
